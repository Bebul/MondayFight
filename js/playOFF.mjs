...
import {addNewGamesStats, addStats} from "./analyze.mjs"
...
import {getHistogram} from "./cross.mjs";

let playOFF = {
  '2021': 'MiX9k7Eo,EDVuU1Dw,5l6Lsnzc,c3o88IoF,uPfXwttU,' + //bebul
     ...
     'zt5RXbWC,KlfuO36G,Q9MssTxq,tvjFtefk,DT9aovti', //dj-pesec
  ...
  tournamentSkeleton: {
    ...
    "fullName": "Monday fight PlayOFF 2021",
    "id": "playOFF2021",
    "createdBy": "jouzolean",
...
async function extractTournamentData(data, tournament, games, playOFFId) {
  // sort games by date
  ...
  tournament.fullName = `Monday Fight PlayOFF ${playOFFId}`
  ...
  let gamesJSON = JSON.stringify({id: tournament.id, games: games})
...
export async function createPlayOFFTournament(data, playOFFId) {
  let theGames = playOFF['dwnl'+playOFFId] || await LAPI.lichessAPI().games(playOFF[playOFFId])
  let t = JSON.parse(JSON.stringify(playOFF.tournamentSkeleton))
  let tournament = await extractTournamentData(data, t, theGames, playOFFId)

  console.log(theGames.length)
}
